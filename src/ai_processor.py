...
def process_daily_news(articles: list[dict]) -> list[dict]:
    ...
    user_message = (
        ...
        '      "company": "회사명",\n'
        '      "title": "원문 제목 그대로",\n'
        '      "link": "원문 link 그대로 (입력 link 필드 복사)",\n'
        ...
        "3. 제외 대상은 '신상품 단순 홍보' 또는 '시황 종목 추천 글에 회사명 단순 언급'에 한정. 보도 가치가 조금이라도 있으면 포함.\n"
        "4. 동일 보도자료 기반 중복 기사는 1건만 채택. 단, 매체별 시각 차이가 명확하면 별도 채택.\n"
        "5. link 필드는 입력 데이터의 link를 글자 그대로 복사 (수정·재구성 금지).\n"
    ...
    # Diagnostic: naver vs external breakdown after filter
    n_naver = sum(1 for it in filtered if it.get("is_naver"))
    n_ext = sum(1 for it in filtered if not it.get("is_naver"))
    print(f"[INFO] AI filter result: total={len(filtered)} (naver={n_naver}, external={n_ext})", flush=True)
